...
function setInitialParametersForNewPlayer(room_name, socket_id) {
    ...
    // Set ID to next empty COLOR in the room.
    player_data.ID = rooms[room_name].last_ID_given_to_player;

    // Update ID to be ready for the next player.
    rooms[room_name].last_ID_given_to_player+=4;
...
function initNewRoom(room_name, socket_id) {
    ...
    // Set next ID that will be  given to a new  player in room.
    rooms[room_name].last_ID_given_to_player = 2;
